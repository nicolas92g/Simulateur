...
nico::InstancedModel::InstancedModel(const char* path)
{
	loadModel(path);
}

void nico::InstancedModel::setModels( std::vector<glm::mat4>& models)
{
	for (auto& mesh : meshes)
		mesh.setModels(models);
}

void nico::InstancedModel::draw(Shader* shader)
{
	for(auto& mesh : meshes)
		mesh.draw(shader);
}

void nico::InstancedModel::loadModel(const char* path)
{
	...
	{
		std::cout << "ERROR with assimp in the nico::InstancedModel class :\n" << importer.GetErrorString() << "\n";
		return;
	}
...
void nico::InstancedModel::processNode(aiNode* node, const aiScene* scene)
{
...
nico::InstancedMesh nico::InstancedModel::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	return nico::InstancedMesh(vertices, indices, textures);
}

std::vector<nico::Texture> nico::InstancedModel::loadMaterialTextures(aiMaterial* mat)
{
	std::vector<Texture> textures;
