...
int exempleDeProgrammeComplet()
{
	...
	//creation de la montgol
	Model montgolfiereMesh(NICO_PATH"MODELISATION/baloon.obj");//on charge une geometrie 3D depouis un fichier .obj créé avec blender
	Object3d montgolfiere(&montgolfiereMesh, vec3(2, 2, -1));//on créé un object 3d qui a une position, une taille, une rotation et qui a une geometrie 3D
	render.addEntity(&montgolfiere);// on met cet objet 3D dans notre afficheur

	//soleil
	DirectionalLight sun(vec3(0.1, -0.9, 0.3));// ! // on cree une lumière infiniment loin et infiniment puissante avec une direction initiale
	render.setDirectionalLight(&sun);// ! // on met cette lumiere a notre afficheur
	sun.setColor(vec3(1));

	//lumiere
	Light point;//on créé une lumière
	point.setPos(montgolfiere.getPos() + vec3(3, 0.5, 1));//on modifie sa position
	...
	spot.setPos(montgolfiere.getPos() + vec3(-10, -1.5, 6));//on modifie sa position
